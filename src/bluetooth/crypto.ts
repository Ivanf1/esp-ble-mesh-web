...
const getPublicKeyHex = async (key: CryptoKey) => {
  ...
  return utils.arrayBufferToHex(rawKey).substring(2);
};

const importKey = async (hexKey: string) => {
  ...
  const pbk = await window.crypto.subtle.importKey(
    "raw",
    utils.hexToArrayBuffer("04" + hexKey),
    {
      name: "ECDH",
      namedCurve: "P-256",
    },
...
const computeECDHSecret = async (publicKey: CryptoKey, privateKey: CryptoKey) => {
 ...
 */
const makeConfirmationProvisioner = (
  provisioningInvitePDUValue: string,
  provisioningCapabilitiesPDUValue: string,
  provisioningStartPDUValue: string,
  publicKeyProvisioner: string,
  publicKeyDevice: string,
  ecdhSecret: string,
  authValue: string
) => {
  ...
  const confirmationKey = k1(ecdhSecret, "7072636b", confirmationSalt);
  ...
  const confirmationProvisioner = getAesCmac(confirmationKey, randomProvisioner + authValue);
